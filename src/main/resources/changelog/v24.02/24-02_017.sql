...
--mise à jour de l'ajout d'image

DROP PROCEDURE IF EXISTS public.opentheso_add_external_images(character varying, character varying, integer, text);
CREATE OR REPLACE PROCEDURE public.opentheso_add_external_images(
...
-- mise à jour de la récupération d'images 
DROP FUNCTION IF EXISTS public.opentheso_get_images(character varying, character varying);
CREATE OR REPLACE FUNCTION public.opentheso_get_images(
	id_theso character varying,
	id_con character varying)
    RETURNS TABLE(name character varying, copyright character varying, url character varying, creator character varying) 
    LANGUAGE 'plpgsql'
    COST 100
...
-- mise à jour de la récupération du concept pour les images
