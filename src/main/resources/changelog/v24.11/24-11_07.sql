...
-- FUNCTION: public.opentheso_get_concepts(character varying, character varying)
...
FOR group_rec IN SELECT * FROM opentheso_get_groups(id_theso, con.id_concept)
                                   LOOP
                               -- Log du début de l'itération avec les valeurs de group_id, group_id_ark, et group_id_handle
    RAISE NOTICE 'Traitement de group_id : %, group_id_ark : %, group_id_handle : %', group_rec.group_id, group_rec.group_id_ark, group_rec.group_id_handle;
...
IF (theso_rec.original_uri_is_ark = true AND group_rec.group_id_ark IS NOT NULL AND group_rec.group_id_ark != '') THEN
        ...
        RAISE NOTICE 'Condition 1 (ARK avec URI original), membre : %', membre;
    ...
    ELSIF (theso_rec.original_uri_is_ark = true AND (group_rec.group_id_ark IS NULL OR group_rec.group_id_ark = '')) THEN
        ...
        RAISE NOTICE 'Condition 2 (ARK sans URI), membre : %', membre;
    ...
    ELSIF (group_rec.group_id_handle IS NOT NULL AND group_rec.group_id_handle != '') THEN
        ...
        RAISE NOTICE 'Condition 3 (Handle), membre : %', membre;
    ...
    ELSIF (theso_rec.original_uri IS NOT NULL AND theso_rec.original_uri != '') THEN
        ...
        RAISE NOTICE 'Condition 4 (URI original sans ARK), membre : %', membre;

ELSE
        membre = membre || path || '/?idc=' || group_rec.group_id || '&idt=' || id_theso || seperateur;
        RAISE NOTICE 'Condition 5 (Autre cas), membre : %', membre;
END IF;

    -- Log pour la fin de l'itération
    RAISE NOTICE 'Fin de traitement pour group_id : %, membre actuel : %', group_rec.group_id, membre;
END LOOP;
...
ALTER FUNCTION public.opentheso_get_concepts(character varying, character varying)
    OWNER TO opentheso;
