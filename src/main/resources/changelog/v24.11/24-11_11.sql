DROP FUNCTION IF EXISTS public.opentheso_get_alignements(character varying, character varying);

CREATE OR REPLACE FUNCTION public.opentheso_get_alignements(
	id_theso character varying,
	id_con character varying)
    RETURNS TABLE(alig_uri_target character varying, alig_id_type integer, alig_thesaurus_target character varying)
    LANGUAGE 'plpgsql'
    COST 100
...
begin
return query
SELECT uri_target, alignement_id_type, thesaurus_target
FROM alignement
