    ...
    private boolean setThesaurusData(ReadStruct readStruct, String lang) {
        switch (readStruct.property.getLocalName()) {
            ...
            case "source":
                ...
                return true;
            case "created":
                readStruct.resource.getThesaurus().setCreated(new DateUtils().getDateFromString(readStruct.literal.getLabel()));
                return true;
            case "modified":
                readStruct.resource.getThesaurus().setModified(new DateUtils().getDateFromString(readStruct.literal.getLabel()));
                return true;
        }
        return false;
    }
